...
- **업무기능**: 일반 사용자의 Marketplace에서 Agent 구독 신청 (Developer 승인 필요)
- **흐름 요약**: 로그인(User) → MarketPlace > AgentMarket 접근 → Agent 선택 → 구독 신청 버튼 클릭 → 구독 신청 완료 확인 → My > 구독Agent에서 '승인 대기' 상태 확인
- **검증포인트**: 구독 신청 버튼 활성화 여부 / 신청 후 My > 구독Agent에 '승인 대기' 상태 노출 여부 / 이미 신청 중인 Agent 중복 신청 불가 처리 여부 / Developer > Agent 관리에 구독 신청 노출 여부
...
## 22. Developer 권한 사용자 일반 사용자 구독 신청 승인

- **업무기능**: Developer의 일반 사용자 Agent 구독 신청 승인 처리
- **흐름 요약**: 로그인(Developer) → Developer > Agent 관리 접근 → 구독 신청 목록 확인 → 신청 사용자 선택 → 승인 버튼 클릭 → 승인 완료 확인 → 해당 사용자 My > 구독Agent 상태 '구독 중' 변경 확인
- **검증포인트**: 승인 후 사용자 구독 상태 '구독 중' 변경 여부 / 승인 완료 메시지 출력 여부 / My > 구독Agent에 결과 반영 여부
- **사전 요구사항**: Developer 계정 로그인 상태 / 일반 사용자의 구독 신청 존재 / 서비스 중인 Agent 존재

---

## 23. Developer 권한 사용자 일반 사용자 구독 신청 반려

- **업무기능**: Developer의 일반 사용자 Agent 구독 신청 반려 처리
- **흐름 요약**: 로그인(Developer) → Developer > Agent 관리 접근 → 구독 신청 목록 확인 → 신청 사용자 선택 → 반려 버튼 클릭 → 반려 사유 입력 → 반려 완료 확인 → 해당 사용자 My > 구독Agent 상태 '반려' 변경 확인
- **검증포인트**: 반려 후 사용자 구독 상태 '반려' 변경 여부 / 반려 사유 저장 여부 / My > 구독Agent에 반려 결과 반영 여부
- **사전 요구사항**: Developer 계정 로그인 상태 / 일반 사용자의 구독 신청 존재
